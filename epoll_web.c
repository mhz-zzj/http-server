...
//gcc epoll_web.c wrap.c pub.c thread_pool.c -pthread
...
void read_client_request(nJob *job)
{
	struct MyData data=*(MyData*)job->user_data;
	int epfd = data.epfd;
	struct epoll_event *ev=data.ev;
...
int main(int argc, char const *argv[])
{
	...
	nThreadPool poll;
	ntyThreadPoolCreate(&poll,MAX_THREAD);
		...
		else
		{
			for(int i=0;i<nready;i++)
			{
				...
				else if(evs[i].events & EPOLLIN)//cfd变化
				{
					...
					job->job_function=read_client_request;
					MyData myData={ epfd,&evs[i]};
					job->user_data=malloc(sizeof(MyData));
					*(MyData*)job->user_data=myData;
					ntyThreadPoolQueue(&poll,job);
				}
			}
		}
	}
